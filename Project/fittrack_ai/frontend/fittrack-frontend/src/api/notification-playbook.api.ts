...
export type NotificationPlaybook = { id:string; name:string; category:'WELLNESS'|'MEAL'|'SLEEP'|'PRODUCTIVITY'; mode:'FIXED'|'RANDOM'; triggerTime:string; daysOfWeek:string; messages:string; conditionType:'ANY'|'MEALS_LT'|'PROTEIN_GT'|'NO_MEAL'; threshold:number|null; recipientMode:'ALL_ACTIVE'|'SELECTED'; recipientUserIds:string[]; enabled:boolean; lastTriggeredDate:string|null; createdAt:string };
