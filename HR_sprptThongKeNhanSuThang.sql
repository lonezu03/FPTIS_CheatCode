...
BEGIN
	...
	IF @Activity = 'CheckMissingContract'
	BEGIN
	    -- 1. NV trong #HR_tblDSNV_Group nhưng không có ContractID (thiếu hợp đồng hoàn toàn)
	    SELECT DISTINCT A.EmpID, A.IsEnd
	    FROM #HR_tblDSNV_Group A
	    WHERE A.ContractID IS NULL;

	    -- 2. NV có HĐ nhưng loại không nằm trong các dòng thống kê hiện tại
	    SELECT DISTINCT 
	        A.EmpID,
	        A.IsEnd,
	        A.LSContractTypeCode,
	        B.ContractID,
	        C.LSContractTypeCode AS ContractTypeCode_Real,
	        D.LSContractTypeGroupCode
	    FROM #HR_tblDSNV_Group A
	    LEFT JOIN dbo.HR_tblContract B 
	           ON B.ContractID = A.ContractID
	    LEFT JOIN dbo.LS_tblContractType C 
	           ON C.LSContractTypeID = B.LSContractTypeID
	    LEFT JOIN dbo.LS_tblContractTypeGroup D 
	           ON D.ContractTypeGroupID = C.[Type]
	    WHERE 
	        ISNULL(A.LSContractTypeCode, '') NOT IN (
	            'HDKXDTH','HD03','HD04','HD05',
	            'HDLDCT','HDTV','HD12','HD11','HD10'
	        )
	        AND (D.LSContractTypeGroupCode IS NULL 
	             OR D.LSContractTypeGroupCode <> 'NHD03');

	    -- 3. NV có > 2 HĐ thuê ngoài (HD03/HD04/HD05)
	    SELECT 
	        A.EmpID,
	        A.IsEnd,
	        COUNT(DISTINCT A.ContractID) AS SoHD_ThueNgoai
	    FROM #HR_tblDSNV_Group A
	    WHERE A.LSContractTypeCode IN ('HD03','HD04','HD05')
	    GROUP BY A.EmpID, A.IsEnd
	    HAVING COUNT(DISTINCT A.ContractID) > 2;

	    -- 4. So sánh số NV subset đầu kỳ vs số NV thực tế được cộng ở 3 dòng đầu (HDKXDTH + thuê ngoài)
	    IF OBJECT_ID('tempdb..#NV_Head_Subset') IS NOT NULL DROP TABLE #NV_Head_Subset;
	    SELECT DISTINCT EmpID
	    INTO #NV_Head_Subset
	    FROM #HR_tblDSNV_Group
	    WHERE IsEnd = 0
	      AND LSContractTypeCode IN ('HDKXDTH','HD03','HD04','HD05');

	    IF OBJECT_ID('tempdb..#NV_Contract_Subset') IS NOT NULL DROP TABLE #NV_Contract_Subset;
	    SELECT DISTINCT EmpID
	    INTO #NV_Contract_Subset
	    FROM (
	        -- Rank 1: HDKXDTH
	        SELECT EmpID
	        FROM #HR_tblDSNV_Group
	        WHERE IsEnd = 0 AND LSContractTypeCode = 'HDKXDTH'

	        UNION

	        -- Rank 2: HD03/HD04/HD05 có đúng 1 HĐ
	        SELECT A.EmpID
	        FROM #HR_tblDSNV_Group A
	        JOIN (
	            SELECT EmpID, IsEnd
	            FROM #HR_tblDSNV_Group
	            WHERE IsEnd = 0 AND LSContractTypeCode IN ('HD03','HD04','HD05')
	            GROUP BY EmpID, IsEnd
	            HAVING COUNT(ContractID) = 1
	        ) B ON A.EmpID = B.EmpID AND A.IsEnd = B.IsEnd

	        UNION

	        -- Rank 3: HD03/HD04/HD05 có đúng 2 HĐ
	        SELECT A.EmpID
	        FROM #HR_tblDSNV_Group A
	        JOIN (
	            SELECT EmpID, IsEnd
	    ...
	    -- Đếm số NV ở 2 bên
	    SELECT COUNT(*) AS SoNV_Headcount_Subset
	    FROM #NV_Head_Subset;

	    SELECT COUNT(*) AS SoNV_Contract_Subset
	    FROM #NV_Contract_Subset;

	    -- NV có trong DSNV subset nhưng không có trong subset được cộng ở 3 dòng đầu
	    SELECT EmpID AS EmpID_OnlyInHeadcount
	    FROM #NV_Head_Subset
	    EXCEPT
	    SELECT EmpID
	    FROM #NV_Contract_Subset;

	    -- NV có trong subset 3 dòng đầu nhưng không nằm trong DSNV subset (thường = 0)
	    SELECT EmpID AS EmpID_OnlyInContract
	    FROM #NV_Contract_Subset
	    EXCEPT
	    SELECT EmpID
	    FROM #NV_Head_Subset;
